...
description: >-
  Separating domain logic from its adapters, so the core can be tested
...
metadata:
  ...
  output: source.py
  origin: ECC
  imported-from: 04-engineering-practices/hexagonal-architecture
---
...
Separating domain logic from its adapters, so the core can be tested without
...
**Asked:** “Restructure this so the calculation can be tested without the database.”
