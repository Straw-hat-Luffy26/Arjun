...
description: >-
  Scoring competitors on a weighted nine-dimension rubric, so a comparison
  rests on stated criteria rather than impressions.
version: 1.0.0
license: MIT
author: unknown
network: none
...
metadata:
  approval-class: none
  output: benchmark_scores.xlsx
  imported-from: 03-evaluation-research/benchmark-methodology
---
...
`benchmark_scores.xlsx`, written into this run's workspace and nowhere else.
...
**Asked:** “Score these four vendors on the rubric so the comparison is defensible.”
...
3. `create_xlsx` to produce `benchmark_scores.xlsx`
