...
name: source-pdf
description: >-
  Read a PDF page by page, using its own text layer where that text is sound
  and local OCR where it is missing or suspect, and report the pages neither
  could resolve.
version: 1.0.0
license: Apache-2.0
author: ARJUN
...
metadata:
  approval-class: none
  for-input: pdf, pdf-text, pdf-scan, pdf-mixed
---
...
A selected source is a PDF. Assess **each page separately** -- a PDF is not one
kind of document, it is a container, and a single file routinely holds native
text, scanned images and vector drawings on consecutive pages.

For each page:

- **Native text that reads as prose** -- use it. It is exact, and OCR of a page
  that already has text is a way to introduce errors.
- **No text, or text that is gibberish, or a text layer far shorter than the page
  it covers** -- treat as image content and apply local OCR and layout recovery.
  A short or garbled layer is the signature of a bad embedded font, or a scanner
  that wrote a token layer over an image.
- **Neither works** -- report the page as unresolved, by number.

Preserve reading order, page labels, heading structure, table context, captions,
and the regions evidence came from, so a citation lands where the reader looks.

## When not to use this

- The source is not a PDF. Use the skill for its actual type.
- The question is only about a figure and `knowledge.multimodal_retrieve` is
  already returning the region. Reading every page first spends the window on
  pages nobody asked about.

**Do not assume a PDF has been fully read because some pages yielded text.** The
completeness record names the pages that produced nothing; carry it through.
...
Every claim carries a page, and a region when the evidence is visual. Alongside
the prose:

- `pagesRead`, `pagesUnresolved` (by number), and how each read page was
  resolved (`text` or `ocr`).
- Table content reported with its surrounding caption and heading, never as a
...
> Pages 1-3 carry a native text layer and were read directly. Page 4 is a scanned
> annexe; OCR recovered its heading and the first two rows of the schedule, and
> the remainder is illegible at this scan quality. Page 5 is a drawing with no
> text layer and no OCR result.
>
> `pagesRead: [1,2,3,4]` (text, text, text, ocr) - `pagesUnresolved: [5]`
