...
3. **The combined recovery acceptance test is failing.** The deterministic journey reaches compaction, two Node-worker restarts, restoration of the same approval, and one file write. It fails at `src-tauri/tests/agent_runtime_durable_journey.rs:187`: one plan step is unfinished and the answer has not been checked. This demonstrates the verifier refusing incomplete evidence; it does not by itself prove the checkpoint machinery failed. The journey must exercise genuine plan completion and answer verification. It currently bypasses the Tauri driver, router, UI, and real inference.
...
| Completion gating | `src-tauri/src/commands/agent.rs:1999,2056` invokes verification and applies its verdict to the final outcome. The combined journey still needs a passing demonstration of completed and checked work. |
...
1. Finish the deterministic recovery journey through genuine plan and answer verification, including a test through the production task driver.
